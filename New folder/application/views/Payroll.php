...
<input type="text" name="department" id="department" class="form-control">
						...
						<table class="table datatable table-bordered" id="myTable">
						    ...
						    <tr id="refreshData">
							 <!--<input type="hidden" id="data" name="tabledata" value="<?php echo $row['ID'];?>">-->
								...
								<td><center></center></td>
								<td id="name"><center></center></td>
								<td id="month"><center></center></td>
								<td id="dep"><center></center></td>
								<td id="pay_no"><center></center></td>
								<td id="total"><center></center></td>
								<td id="work"><center></center></td>
								<td id="basic"><center></center></td>
								<td id="increment"><center></center></td>
								<td id="incentive"><center></center></td>
								<td id="TA"><center></center></td>
								<td id="HRA"><center></center></td>
								<td id="DA"><center></center></td>
								<td id="gross_ear"><center></center></td>
								<td id="LOP"><center></center></td>
								<td id="adv_salary"><center></center></td>
								<td id="ot_edu"><center></center></td>
								<td id="gross_deduc"><center></center></td>
								<td id="remarks"><center></center></td>
								<td id="net_amt"><center></center></td>
 ...
 <script>
	function Save_Pdf_generate(){
		...
		var ta=$("#TA").val();
		var advance_salary=$("[id='Advance salary if paid']").val();
		var other_deduction=$("[id='Other Deduction']").val();
	...
	function save_pdf_email(){
		...
		$.ajax({
		    ...
		    data:{emp_name:emp_name,emp_id:emp_id,department:department,emp_email:emp_email,payslip_no:payslip_no,basic_salary:basic_salary,month_year:month,
...
function net_amount(){
 ...
 $.ajax({
  ...
  $("#tworking_days").val(json.disp1[0].CL_WRK_DAYS);
	...
	$("#myModal2").modal('show');
	if (json!='') {
	    $("#name").text(json.history[0].EMP_NAME);
	    $("#month").text(json.history[0].MONTH_YEAR);
	    $("#dep").text(json.history[0].EMP_DEPARTMENT);
	    $("#pay_no").text(json.history[0].PAYSLIP_NO);
	    $("#total").text(json.history[0].TOTAL_WORKING_DAYS);
	    $("#work").text(json.history[0].WORKED_DAYS);
	    $("#basic").text(json.history[0].BASIC_SALARY);
	    $("#increment").text(json.history[0].INCREMENT);
	    $("#incentive").text(json.history[0].INCENTIVE);
	    $("#TA").text(json.history[0].TRAVELLING_ALLOWANCE);
	    $("#HRA").text(json.history[0].HRA);
	    $("#DA").text(json.history[0].DA);
	    $("#gross_ear").text(json.history[0].GROSS_EARNINGS);
	    $("#LOP").text(json.history[0].LOP);
	    $("#adv_salary").text(json.history[0].ADVANCE_SALARY);
	    $("#ot_edu").text(json[0].OTHER_DEDUCTIONS);
	    $("#gross_deduc").text(json.history[0].GROSS_DEDUCTIONS);
	    $("#remarks").text(json.history[0].REMARKS);
	    $("#net_amt").text(json.history[0].NET_AMOUNT);
